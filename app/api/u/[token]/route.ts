...
function page(title: string, message: string, action = ""): NextResponse {
  ...
  return new NextResponse(
    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>body{margin:0;background:#f7f5f2;color:#1d1b18;font:16px/1.55 system-ui,sans-serif}main{box-sizing:border-box;max-width:560px;margin:12vh auto;padding:32px;border:1px solid #e8e3dc;border-radius:18px;background:#fff;box-shadow:0 18px 50px rgba(40,32,24,.10)}h1{margin:0 0 12px;font-size:28px}p{color:#6b655e}button{min-height:44px;margin-top:12px;padding:0 20px;border:0;border-radius:10px;background:#72506f;color:#fff;font:inherit;font-weight:650;cursor:pointer}button:hover{background:#5e405b}button:focus-visible{outline:3px solid #c7a8c4;outline-offset:3px}</style></head><body><main><h1>${title}</h1><p>${message}</p>${form}</main></body></html>`,
