...
function page(title: string, message: string, action = ""): NextResponse {
  ...
  return new NextResponse(
    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><style>body{margin:0;background:#f5f7fb;color:#182033;font:16px/1.55 system-ui,sans-serif}main{box-sizing:border-box;max-width:560px;margin:12vh auto;padding:32px;border:1px solid #dce2ea;border-radius:18px;background:#fff;box-shadow:0 18px 50px rgba(24,32,51,.08)}h1{margin:0 0 12px;font-size:28px}p{color:#566176}button{min-height:44px;margin-top:12px;padding:0 20px;border:0;border-radius:10px;background:#235ee7;color:#fff;font:inherit;font-weight:650;cursor:pointer}button:focus-visible{outline:3px solid #8db2ff;outline-offset:3px}</style></head><body><main><h1>${title}</h1><p>${message}</p>${form}</main></body></html>`,
